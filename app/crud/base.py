...
class CRUDBase(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    async def get_by_user_id(self, db: AsyncSession, user_id: uuid.UUID) -> Optional[ModelType]:
        result = await db.execute(select(self.model).filter(self.model.user_id == user_id))
        return result.scalars().first()

    async def get_by_telegram_id(self, db: AsyncSession, telegram_id: int) -> Optional[ModelType]:
